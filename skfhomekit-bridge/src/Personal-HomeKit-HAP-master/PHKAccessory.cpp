...
string AccessorySet::describe() {
    ...
    printf("accessoryset:describe:%s",result.c_str());
    return result;
}
...
void updateValueFromDeviceEnd(characteristics *c, int aid, int iid, string value) {
    //c->setValue(value);  //Removing for status update from rocker switches like scinarios but Fix with by muting this event
                ...
                if (a==NULL) {
                    ...
                    if (updateNotify) {
                        ...
                        if (c==NULL) {
                            ...
                            if (c->writable()) {
                                c->setValue(value);    //seems to setting the value here
                                ...
                                info->desc = broadcastTemp;
                                pthread_t thread;
                                //pthread_create(&thread, NULL, announce, info);  seems this is instance where values updates synchronosly let dive into it
                                statusCode = 204;
                                string fff;
