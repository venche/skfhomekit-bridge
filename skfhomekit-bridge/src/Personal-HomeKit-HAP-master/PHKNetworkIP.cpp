...
void broadcastMessage(void *sender, char *resultData, size_t resultLen) {
    ...
    for (int i = 0; i < numberOfClient; i++) {
        ...
        //printf("socketnum:%d\tnotify:%d\n ",socketNumber,connection[i].notify(sender)     );
        if (socketNumber >= 0 && connection[i].notify(sender)) {
            ...
            connection[i].numberOfMsgSend++; //changing as per original
...
void connectionInfo::handleAccessoryRequest() {
    ...
    do {
        ...
        if (len > 0) {
            ...
            /*decryptData :- Request: GET /characteristics?id=1.9 HTTP/1.1
            Host: SILVAN\032HOMEKIT\032Bridge._hap._tcp.local
            ...
            //bzero(temp, 64); bzero(temp2, 64);   //added by me surprisingly broadcast is working after this wondering
            // printf("Resetting buffers of temp & temp32\n"   );
            chacha20_encrypt(&chacha20, (const uint8_t*)temp, (uint8_t *)temp2, 64); //seems to poly encrypt
            chacha20_encrypt(&chacha20, (const uint8_t*)resultData, (uint8_t*)&reply[2], resultLen);
            ...
            printf("Replyback\n");
            write(subSocket, reply, resultLen+18); //writing  response back
